...
func SaveConfig(v []byte) error {
	...
	if err != nil {
		return fmt.Errorf(
			"organiStash needs to be able to create/edit its own configuration which it can't at the moment. "+
				"Change the permission for organiStash to create and edit `%s`",
			config_path(),
		)
	}
